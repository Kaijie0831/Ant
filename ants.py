...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place, move = self.place, 0
        while not place.is_hive and move <= self.upper_bound:
            if place.bees and self.lower_bound <= move:
                return random_bee(place.bees)
            else:
                place, move = place.entrance, move + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            total_damage = self.health + self.damage
        else:
            total_damage = amount
        for each in self.place.bees[:]:
            each.reduce_health(total_damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            bee_get_hurt = random_bee(self.place.bees)
            if bee_get_hurt:
                bee_get_hurt.reduce_health(bee_get_hurt.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.ant_contained:
            self.ant_contained.action(gamestate)
        for each in self.place.bees[:]:
            each.reduce_health(self.damage)
# END Problem 9
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while loc.exit:
            if loc.exit.ant:
                if loc.exit.ant.is_container and loc.exit.ant.ant_contained:
                    if not loc.exit.ant.ant_contained.is_doubled:
                        loc.exit.ant.ant_contained.double()
                if not loc.exit.ant.is_doubled:
                    loc.exit.ant.double()
            loc = loc.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        if target is not None:
            if not hasattr(target, 'origin_action'):
                target.origin_action = target.action
            target.slow_turns = 5
                
            def slow_action(gamestate):
                if 'Scary' in target.effect_dict:
                    target.origin_action = target.effect_dict['Scary']
                else:
                    target.origin_action = target.effect_dict['default']
                if target.slow_turns <= 0:
                    target.origin_action(gamestate)
                    target.action = target.origin_action
                    del target.effect_dict['Slow']
                else:
                    if not (gamestate.time % 2):
                        target.origin_action(gamestate)
                    target.slow_turns -= 1
            
            if 'Slow' not in target.effect_dict:
                target.effect_dict['Slow'] = slow_action
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        if target is not None and target.remaining_scared_turns:
            target.scare(2)
        # END Problem EC 2
...
class Bee(Insect):
    ...
    damage = 1
    is_waterproof = True
    remaining_scared_turns = True
    
    def __init__(self, health=1):
        super().__init__(health)
        self.default_action = self.action
        self.effect_dict = {'default': self.default_action}
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC 2
        if self.remaining_scared_turns: 
            self.remaining_scared_turns = length
            
            def scared_action(gamestate):
                if self.remaining_scared_turns and self.place.entrance and not self.place.entrance.is_hive:
                    if self.blocked():
                        self.sting(self.place.ant)
                    self.move_to(self.place.entrance)
                    self.remaining_scared_turns -= 1
                    if self.remaining_scared_turns == 0:
                        if 'Slow' in self.effect_dict:
                            del self.effect_dict['Scary']
                        else:
                            self.action = original_action
                else:
                    original_action(gamestate)
            
            self.effect_dict['Scary'] = scared_action
            if 'Slow' in self.effect_dict:
                self.action = self.effect_dict['Slow']
            else:
